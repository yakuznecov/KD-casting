(function ($) {
  $(document).on('ready', function () {
    $('.casting__training-slider').slick({
      ...
      cssEase: 'linear',
      prevArrow:
        '<button type="button" data-role="none" class="secondary-btn --inactive slick-prev slick-arrow" aria-label="Previous" role="button" >Назад</button>',
      nextArrow:
        '<button type="button" data-role="none" class="primary-btn slick-next slick-arrow" aria-label="Next" role="button" >Вперёд</button>',
    });
  });
  ...
  $(document).on('ready', function () {
    $(function () {
      ...
      const displayScore = function () {
        const sliderItem =
          $('.casting__training-slider-item.slick-active').data('slick-index') +
          1;
  ...
  function transformMainHeaderBtn() {
    ...
    $(window).on('load resize', function () {
      let mainLeft =
        mainHeaderBottomLeft.offset().left + mainHeaderBottomLeft.width() + 56;
      let sendLeft = sendInvitationBtn.offset().left; // кнопка
      let statusleft = statusDefault.offset().left;
      let hideMediaStatusLeft =
        hideMediaStatus.offset().left + hideMediaStatus.width();
      ...
      if (
        mainHeaderTopSearch.hasClass('lower') &&
        topSearchLeft - mainLeft < 210
      ) {
        smallSendBtn();
      }
  ...
  function rangeFilterSlider() {
    ...
    const updateView = function () {
      ...
      const inclRangeWidthFirst =
        ((rangeOne.val() - rangeTwo.val()) / this.attr('max')) * 100 + '%';
      const inclRangeWidthSecond =
        ((rangeTwo.val() - rangeOne.val()) / this.attr('max')) * 100 + '%';
      const inclRangeLeftFirst =
        (rangeTwo.val() / this.attr('max')) * 100 + '%';
      const inclRangeLeftSecond =
  ...
  $('.castings-menu-agent, .promo-menu-actors, .castings-menu, .promo-menu').on(
    'mouseover',
    function () {
